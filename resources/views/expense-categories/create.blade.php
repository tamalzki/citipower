...
@section('content')
    <div class="page-header">
        <div>
            ...
            <p>Create a category for organizing expenses</p>
        </div>
        ...
        <div class="card-body">
            <form action="{{ route('expense-categories.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-control" value="{{ old('name') }}" placeholder="e.g. Utilities" required autofocus>
                    @error('name') <small style="color:#dc2626; font-size:11px; display:block; margin-top:4px;">{{ $message }}</small> @enderror
                </div>

                <div class="form-group">
                    <label>Description</label>
                    <textarea name="description" class="form-control" rows="3" placeholder="Optional category description">{{ old('description') }}</textarea>
